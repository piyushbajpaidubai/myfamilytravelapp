...
// The itinerary preview deliberately renders the production ScheduleTab. This keeps
// the review URL and the signed-in app on the same UI instead of maintaining two
// visually similar implementations that can drift apart.
function IntegratedItineraryPreview() {
  const [trip, setTrip] = useState(() => JSON.parse(JSON.stringify(NATIVE_ITINERARY_DEMO)));
  const update = patch => setTrip(current => {
    const next = typeof patch === 'function' ? patch(current) : patch;
    return { ...current, ...(next || {}) };
  });
  const session = { userId:trip.members?.[0]?.userId || 'demo-user' };

  return (
    <div style={{ minHeight:'100vh', background:'#D8C6A6', padding:'18px 0 70px' }}>
      <ScheduleTab trip={trip} update={update} session={session} canEdit />
    </div>
  );
}
                ...
                {!collapsed && <div style={{ position:'relative' }}><span aria-hidden="true" style={{ position:'absolute',left:17,top:20,bottom:24,width:1,background:'#D8CFC4' }}/>
                  {items.map(({kind,item},itemIndex) => {
                    ...
                    return <article key={event.id} style={{ display:'grid',gridTemplateColumns:'36px minmax(0,1fr)',gap:10,position:'relative',marginBottom:itemIndex===items.length-1?0:12 }}>
                      <span aria-hidden="true" style={{ width:13,height:13,margin:'19px 0 0 11px',padding:0,borderRadius:'50%',background:STATUS_META[status].ring,border:'3px solid #F7F5F0',boxShadow:`0 0 0 1px ${STATUS_META[status].ring}`,boxSizing:'border-box',zIndex:2 }}/>
                      <div style={{ background:isSpan?'#FFF9ED':'#fff',border:`1px solid ${isSpan?'#E8D7AF':'#E8E1D8'}`,borderRadius:18,boxShadow:'0 4px 14px rgba(63,47,40,0.05)',overflow:'hidden' }}>
                        <button type="button" aria-label={`Update ${event.title} status. Current status: ${statusLabel[status]}`} onClick={()=>{ const next=nextStatus(status); isSpan?cycleSpanStatus(event.id):cycleItemStatus(day.id,event.id); setToast(`${event.title}: ${statusLabel[next]}`); }} style={{ width:'100%',border:'none',background:'transparent',padding:'13px 13px 12px',textAlign:'left',cursor:'pointer',display:'block' }}>
                        ...
                        <button type="button" aria-expanded={expanded} aria-label={`${expanded?'Collapse':'Expand'} ${event.title} details`} onClick={()=>setExpandedId(expanded?null:event.id)} style={{ width:'100%',minHeight:46,border:'none',borderTop:'1px solid #D8C8B8',background:expanded?'#E4D7C8':'#E9DED1',padding:'8px 13px',textAlign:'left',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'space-between',gap:8 }}>
                          <span style={{ display:'flex',alignItems:'center',minWidth:0 }}>{members.slice(0,event.assignees&&event.assignees.length?Math.min(4,event.assignees.length):4).map((member,index)=><span key={member.userId} style={{ width:25,height:25,marginLeft:index===0?0:-6,borderRadius:'50%',overflow:'hidden',border:'2px solid #fff',boxShadow:'0 0 0 1px #D9D0C6',flexShrink:0 }}><img src={member.pic} alt="" style={{ width:'100%',height:'100%',objectFit:'cover' }}/></span>)}<span style={{ marginLeft:7,color:'#907D73',fontSize:10.5,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{event.assignees&&event.assignees.length?`${event.assignees.length} assigned`:'Everyone'}</span></span>
                          <span style={{ color:'#6E2118',transform:expanded?'rotate(180deg)':'none',transition:'transform .15s',display:'grid',placeItems:'center',flexShrink:0 }}><NativeStatusIcon name="chevron" size={15}/></span>
                        </button>
                        {expanded && <div style={{ padding:'0 12px 13px',borderTop:'1px solid #F0EBE5',background:'#FCFAF6' }}>
...
export default function Root() {
  ...
  if (demo === 'native-itinerary') return <IntegratedItineraryPreview />;
