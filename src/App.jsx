    ...
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.3)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center" }}>
      <div style={{ background:"#F0EBE0",borderRadius:12,padding:24,minWidth:320,maxWidth:480,width:"90%",boxShadow:"0 8px 32px rgba(44,24,16,0.15)" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
          ...
          <button onClick={onClose} style={{ background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#B54030" }}>Ã</button>
        </div>
        {children}
      </div>
    </div>
    ...
    <div style={{ marginTop:6 }}>
      {docs.map(doc => (
        <div key={doc.id} style={{ display:'flex',alignItems:'center',gap:6,padding:'3px 0',borderBottom:'1px solid #E8E2D4' }}>
          ...
          <button onClick={()=>onDel(doc.id)} style={{ background:'none',border:'none',cursor:'pointer',color:'#C04428',fontSize:13,padding:'0 2px',lineHeight:1 }}>â</button>
        </div>
      ))}
            ...
            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',background:'#3D0C02',color:'#fff' }}>
              ...
              <div style={{ display:'flex',gap:8,flexShrink:0 }}>
                ...
                <button onClick={closePreview}
                  style={{ background:'rgba(255,255,255,0.15)',border:'none',borderRadius:6,color:'#fff',cursor:'pointer',fontSize:16,width:28,height:28,lineHeight:'28px',textAlign:'center',padding:0 }}>Ã</button>
              </div>
            </div>
            {/* Preview pane */}
    ...
    <div>
        ...
        <div key={day.id} style={{ marginBottom:16,border:"1px solid #D4BFB0",borderRadius:10,overflow:"hidden",background:"#EDE7D9" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:"#DDD8CB" }}>
            ...
            <div style={{ display:"flex",gap:6 }}>
              ...
              <Btn variant="danger" style={{ padding:"4px 8px",fontSize:12 }} onClick={()=>delDay(day.id)}>â</Btn>
            </div>
          </div>
            ...
            <div key={ev.id} style={{ padding:"10px 14px",borderTop:"1px solid #D4BFB0" }}>
              ...
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
                <div style={{ flex:1 }}>
                  ...
                  {ev.location && <div style={{ fontSize:12,color:"#A83020",marginTop:2 }}>ð {ev.location}</div>}
                ...
                <label title="Attach document" style={{ display:'inline-flex',alignItems:'center',justifyContent:'center',width:26,height:26,borderRadius:6,cursor:'pointer',color:'#8B2A14',background:'rgba(139,42,20,0.08)',flexShrink:0,marginLeft:6 }}>
                  <span style={{ fontSize:15, lineHeight:1 }}>ð</span>
                ...
                <Btn variant="danger" style={{ padding:"2px 8px",fontSize:12,flexShrink:0,marginLeft:8 }} onClick={()=>delEvent(day.id,ev.id)}>â</Btn>
              </div>

              {/* ââ Documents for Event ââ */}
              <DocList
                docs={ev.docs||[]}
                  ...
                  {(ev.activities||[]).map(act => (
                    <div key={act.id} style={{ marginBottom:6 }}>
                      <div style={{ display:"flex",alignItems:"flex-start",gap:6 }}>
                        <span style={{ fontSize:12,color:"#8B2A14",marginTop:1 }}>â¸</span>
                        <div style={{ flex:1 }}>
                        ...
                        <span style={{ fontSize:12, lineHeight:1 }}>ð</span>
                      ...
                      <button onClick={()=>delActivity(day.id,ev.id,act.id)} style={{ background:'none',border:'none',cursor:'pointer',color:'#C04428',fontSize:12,padding:'0 2px',lineHeight:1,flexShrink:0,marginTop:2 }}>â</button>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {showEvent && (
        <Modal title="Add Event" onClose={()=>setShowEvent(null)}>
          ...
          <Input label="Notes" value={evForm.notes} onChange={e=>setEvForm({...evForm,notes:e.target.value})} placeholder="Any notesâ¦" />
          <div style={{ display:"flex",gap:8,marginTop:8 }}>
          ...
          {pics.map(pic => (
            <div key={pic.id} style={{ position: 'relative', borderRadius: '10px', overflow: 'hidden', aspectRatio: '1', background: '#E8E4D9', cursor: 'pointer', boxShadow: '0 2px 8px rgba(61,12,2,0.12)' }}
              ...
              <button onClick={e => { e.stopPropagation(); delPic(pic.id); }}
                style={{ position: 'absolute', top: '6px', right: '6px', background: 'rgba(61,12,2,0.75)', border: 'none', borderRadius: '50%', width: '24px', height: '24px', color: '#fff', cursor: 'pointer', fontSize: '14px', lineHeight: '24px', textAlign: 'center', padding: 0 }}>Ã</button>
        ...
        <div onClick={() => setLightbox(null)}
          ...
          <div onClick={e => e.stopPropagation()} style={{ position: 'relative', maxWidth: '90vw', maxHeight: '90vh', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.6)' }}>
            ...
            <button onClick={() => setLightbox(null)}
              style={{ position: 'absolute', top: '10px', right: '10px', background: 'rgba(0,0,0,0.6)', border: '2px solid #fff', borderRadius: '50%', width: '32px', height: '32px', color: '#fff', cursor: 'pointer', fontSize: '18px', lineHeight: '28px', textAlign: 'center', padding: 0 }}>Ã</button>
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  if (loading) return <div style={{ display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",color:"#C86050",fontSize:14 }}>Loadingâ¦</div>;
  ...
  return (
    <div style={{ fontFamily:"'Jost','Futura PT','Century Gothic','Trebuchet MS',sans-serif",maxWidth:680,margin:"0 auto",minHeight:"100vh",background:"#F0EBE0" }}>
      ...
      <div style={{ padding:"20px 20px 0",borderBottom:"2px solid #B5341C" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
          <h1 style={{ margin:0,fontSize:20,fontWeight:700 }}>âï¸ My Travel Hub</h1>
          ...
          >
            {savedStatus === 'saved' ? 'â Saved' : savedStatus === 'saving' ? '...' : 'ð¾ Save'}
          </button>
        </div>
                {/* Header note textarea */}
        <textarea value={headerNote} onChange={e=>setHeaderNote(e.target.value)} placeholder="Add a note, quote or travel taglineâ¦" rows={2} style={{ width:"100%",boxSizing:"border-box",resize:"vertical",padding:"8px 12px",marginBottom:12,border:"1px solid #D4BFB0",borderRadius:8,background:"#F0EBE0",color:"#6E1A10",fontSize:13,fontFamily:"inherit",outline:"none",lineHeight:1.5 }} />
        {/* Trip tabs */}
